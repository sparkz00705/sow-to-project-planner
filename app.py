...
def main() -> None:
    ...
    if st.button("🧪 Load Complex Enterprise SOW", use_container_width=False):
        st.session_state["sample_loaded"] = True
        st.session_state["pasted_sow"] = SAMPLE_SOW.strip()
    ...
    with right:
        pasted = st.text_area(
            "Or paste SOW text",
            value=st.session_state.get("pasted_sow", ""),
            key="pasted_sow",
            height=300,
            placeholder="Paste the SOW here or load the built-in complex sample...",
        )
